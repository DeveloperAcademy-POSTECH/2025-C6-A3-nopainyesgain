    ...
    private func downloadAllImages(completion: @escaping (Result<KeyringImages, Error>) -> Void) {
        Task {
            do {
                ...
                // Chain 이미지들 다운로드 (병렬)
                let chainLinks = currentChainType.createChainLinks(length: 6)
                ...
                // Body 이미지 다운
                var downloadedBodyImage: UIImage?
                if let bodyImageURL = self.bodyImage {
                    downloadedBodyImage = try await StorageManager.shared.getImage(path: bodyImageURL)
                }
